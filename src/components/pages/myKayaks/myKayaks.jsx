      ...
      <div className="container mx-auto px-4 py-12 max-w-7xl">
        <div className="mb-10 relative">
          <div className="relative">
            <h1 className="text-4xl font-extrabold text-[#003459] tracking-tight dark:text-white">{translate("Mis Kayaks")}</h1>
            <p className="text-gray-500 dark:text-gray-300 max-w-2xl mt-4">{translate("Explora y administrá tu colección personal de kayaks. Mantené un registro detallado de cada embarcación.")}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center mt-8">
          ...
          <div className="relative flex flex-col items-center justify-center">
            <button className=" flex items-center px-4 py-3 bg-white dark:bg-[#003459] border border-gray-200 rounded-xl hover:bg-gray-50 dark:hover:bg-[#003459] dark:text-white hover:cursor-pointer shadow-sm transition-colors" onClick={() => setFilterOpen(!filterOpen)}>
              ...
              <span className="text-gray-700 dark:text-white font-medium">{translate("Filtros")}</span>
              <ChevronDown
            ...
            {filterOpen && (
              <div className="md:absolute right-0 w-72 bg-white dark:bg-[#003459] border border-gray-100 rounded-xl shadow-lg z-10 p-5">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-white mb-4 mt-4">{translate("Color")}</label>
                    <select className="w-full border border-gray-200 rounded-lg p-2.5 text-sm bg-white dark:bg-[#003459] dark:text-white focus:ring-2 focus:ring-[#007178] focus:border-transparent cursor-pointer">
                      <option value="">{translate("Todos los colores")}</option>
                      <option value="rojo">{translate("Rojo")}</option>
                      <option value="azul">{translate("Azul")}</option>
                      <option value="verde">{translate("Verde")}</option>
                      <option value="amarillo">{translate("Amarillo")}</option>
                      <option value="naranja">{translate("Naranja")}</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-white mb-4 mt-4">{translate("Material")}</label>
                    <select className="w-full border border-gray-200 rounded-lg p-2.5 text-sm bg-white dark:bg-[#003459] dark:text-white focus:ring-2 focus:ring-[#007178] focus:border-transparent cursor-pointer">
                      <option value="">{translate("Todos los materiales")}</option>
                      <option value="polietileno">{translate("Polietileno")}</option>
                      <option value="fibra">{translate("Fibra de vidrio")}</option>
                      <option value="kevlar">{translate("Kevlar")}</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-white mb-4 mt-4">{translate("Capacidad")}</label>
                    <select className="w-full border border-gray-200 rounded-lg p-2.5 text-sm bg-white dark:bg-[#003459] dark:text-white focus:ring-2 focus:ring-[#007178] focus:border-transparent cursor-pointer">
                      <option value="">{translate("Todas las capacidades")}</option>
                      <option value="1">{translate("1 persona")}</option>
                      <option value="2">{translate("2 personas")}</option>
                      <option value="3+">{translate("3 o más personas")}</option>
                    </select>
                  </div>
