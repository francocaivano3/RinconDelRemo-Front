...
const KayaksDisponibles = () => {
  ...
  const handleGetKayaksDisponibles = async () => {
    try {
      ...
      console.log("🚀 ~ handleGetKayaksDisponibles ~ response:", response);
  ...
  const filteredKayaks = kayakList.filter((kayak) => {
    ...
    return (
      nombre.includes(term) || modelo.includes(term) || color.includes(term)
    );
  });
        ...
        <div className="mb-10 relative">
          <div className="relative">
            <h1 className="text-4xl font-extrabold text-[#003459] tracking-tight dark:text-white">
              {translate("Kayaks disponibles para alquilar")}
            </h1>
            <p className="text-gray-500 dark:text-gray-300 max-w-2xl mt-4">
              {translate(
                "Explora y reserva el kayak perfecto para vos y tu familia."
              )}
            </p>
          </div>
        </div>
