        ...
        <div className="dark:bg-[#003459] mx-auto h-screen px-4 py-9">
            <div className="w-full p-4">
                <div className="mb-10 relative">
                    <div className="relative">
                        <h1 className="text-4xl font-extrabold text-[#003459] dark:text-white tracking-tight">{translate("Mis Reservas")}</h1>
                        <p className="text-gray-500 dark:text-gray-300 max-w-2xl mt-4">{translate("Explora tus reservas activas y pasadas.")}</p>
                    </div>
                </div>
            </div>
            <div className="flex flex-col justify-center items-center">
                ...
                <div className="flex justify-between max-w-4xl w-full bg-gray-200 rounded-xl overflow-hidden">
                    <button className={`flex justify-center items-center w-1/2 p-1 ${isPress === 1 ? 'bg-white dark:bg-sky-900 dark:text-white transition-transform duration-500 rounded-xl overflow-hidden shadow-xl border border-gray-200' : 'bg-gray-200 '} `} onClick={() => handleSetIsPress(1)}>{translate("Activas")}</button>
                    <button className={`flex justify-center items-center w-1/2 p-1 ${isPress === 2 ? 'bg-white dark:bg-sky-900 dark:text-white transition-transform duration-500 rounded-xl overflow-hidden shadow-xl border border-gray-200' : 'bg-gray-200 '} `} onClick={() => handleSetIsPress(2)}>{translate("Pasadas")}</button>
                </div>
